...
Func Main()
	GUIDelete()
	Global $Main = GUICreate("SetTimerRes | LuSlower", 195, 250, Default, Default, $WS_POPUP)
	NtQueryTimerResolution()
	_SetProcessInformation()
...
;~ 		"LONG", "SetProcessInformation", _
;~ 		"HANDLE", $hProcess, _
;~ 		"INT", $MEMORY_PRIORITY_CLASS, _
;~ 		"PTR", DllStructGetPtr($MEMORY_PRIORITY_INFORMATION), _
;~ 		"UINT", DllStructGetSize($MEMORY_PRIORITY_SIZE))

;~ Func _MEM_PRIORITY($priority, $process)
;~ If $priority > 5 Or $priority < 1 Then
;~ 	MsgBox(16, "Error", "Wrong priority", 5)
;~ 	Return
;~ EndIf
;~ Switch IsString($process)
;~ 	Case 1
;~ 		Local $PID = ProcessExists($process)
;~ 	Case Else
;~ 		Local $PID = $process
;~ EndSwitch
;~ Local $hProcess = _WinAPI_OpenProcess($PROCESS_SET_INFORMATION, 0, $PID, True) ;SET_INFORMATION is correct
;~ ;Struc MEMORY_PRIORITY
;~ Local $MEMORY_PRIORITY_CLASS = 39
;~ Local $MEMORY_PRIORITY_INFORMATION = DllStructCreate("PTR Priority")
;~ DllStructSetData($MEMORY_PRIORITY_INFORMATION, "Priority", $priority )
;~ Local $MEMORY_PRIORITY_SIZE = DllStructCreate("DWORD Flags")
;~ DllStructSetData($MEMORY_PRIORITY_SIZE, "Flags", 0)
;~ ;Call NtSetInformationProcess
;~ Local $status = DllCall("ntdll.dll", "long", "NtSetInformationProcess", _
;~ 		"HANDLE", $hProcess, _
;~ 		"INT", $MEMORY_PRIORITY_CLASS , _
;~ 		"PTR", DllStructGetPtr($MEMORY_PRIORITY_INFORMATION), _
;~ 		"UINT", DllStructGetSize($MEMORY_PRIORITY_SIZE)) ;4 bytes
;~ _WinAPI_CloseHandle($hProcess) ;Close Handle of Process
;~ ConsoleWrite($status [0] & @CRLF)
;~ EndFunc