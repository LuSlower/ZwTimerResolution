...
BOOL CALLBACK DlgMain(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        hMutex = CreateMutex(NULL, TRUE, "nt_timer");
        if (GetLastError() == ERROR_ALREADY_EXISTS)
        {
        //Ya hay una instancia
        CloseHandle(hMutex);
        MessageBox(0, "Ya hay una instancia de nt_timer ejecutandose", "Error", MB_ICONERROR | MB_OK);
        ...
        char* custom = RegKeyQueryEx(HKEY_CURRENT_USER, "Software\\nt_timer", "CustomTimer");
        ...
        char* start = RegKeyQueryEx(HKEY_CURRENT_USER, "Software\\nt_timer", "Start");
        if (start)
        {
    ...
    UINT state = IsDlgButtonChecked(hwndDlg, _START);
        if (state == BST_CHECKED)
        {
            ...
            if (GetModuleFileName(NULL, path, MAX_PATH) != 0)
                {
                    RegKeySetEx(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run", "nt_timer", path);
                    RegKeySetEx(HKEY_CURRENT_USER, "Software\\nt_timer", "Start", "1");
                }
        }
        else
        {
            RegKeyDelete(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Run", "nt_timer");
            RegKeyDelete(HKEY_CURRENT_USER, "Software\\nt_timer", "Start");
        }
        //exit
